...
function optionLabel(row: BandWithRelations): string {
  const name = row.band_name?.trim() ?? "";
  if (name !== "") return name;
  return `بند #${row.id}`;
}

function toOptions(rows: BandWithRelations[]): BandOption[] {
  return rows.map((r) => ({
    value: r.id,
    label: optionLabel(r),
  }));
...
export type BandSelectLabelPosition = "above" | "inline" | "none";

export type BandSelectProps = {
  value: number | null;
  onChange: (bandId: number | null) => void;
  /** يُحمَّل عبر `getForSelect(babId)` — عند `null` يبقى الحقل غير فعّال */
  babId: number | null;
  disabled?: boolean;
  className?: string;
  placeholder?: string;
  "aria-label"?: string;
  labelPosition?: BandSelectLabelPosition;
};

/**
 * قائمة اختيار للبنود المفعّلة (`getForSelect` يفلتر `status === true`) للباب المحدّد.
 */
export default function BandSelect({
  ...
  className = "",
  placeholder,
  "aria-label": ariaLabel = "البند",
  labelPosition = "above",
}: BandSelectProps) {
  const reactSelectId = useId();
  const { isLoading, data, isError, error } = useBandSelect(
    babId ?? undefined,
  );
  ...
  const handleChange = (opt: SingleValue<BandOption>) => {
    onChange(opt?.value ?? null);
  };

  const hasBab = babId != null;
  const effectivePlaceholder =
    placeholder ?? (hasBab ? "اختر البند" : "اختر الباب أولاً");

  const isDisabled =
    disabled || !hasBab || isLoading || options.length === 0;
  ...
  return (
    <div className={rootClass.trim()} dir="rtl">
      ...
      <Select<BandOption, false>
        ...
        noOptionsMessage={() =>
          hasBab
            ? "لا توجد بنود مفعّلة لهذا الباب"
            : "اختر الباب أولاً"
        }
      ...
      {isError ? (
        ...
        >
          {error instanceof Error
            ? error.message
            : "تعذّر تحميل قائمة البنود"}
        </p>
      ) : null}
      {!isLoading && !isError && hasBab && options.length === 0 ? (
