...
type Props = {
  open: boolean;
  row: CertifiedProgramWithRelations | null;
  isDeleting: boolean;
  onCancel: () => void;
  onConfirm: () => void;
};

export function DeleteCertifiedProgramConfirmDialog({
  open,
  row,
  isDeleting,
  onCancel,
  ...
  return (
    <div
      ...
      role="alertdialog"
      aria-modal="true"
      aria-labelledby="delete-certified-program-title"
      aria-describedby="delete-certified-program-desc"
    >
      <button
        type="button"
      ...
      <div className="relative z-10 w-full max-w-md overflow-hidden rounded-2xl border border-slate-200/90 bg-white shadow-2xl shadow-slate-300/40">
        <div className="flex shrink-0 items-start gap-3 border-b border-red-100 bg-red-50/40 px-4 py-4 sm:px-5">
          ...
          <div className="min-w-0 flex-1 pe-1">
            <h2 id="delete-certified-program-title" className="text-base font-semibold text-slate-900">
              حذف النشاط المعتمد؟
            </h2>
            <p id="delete-certified-program-desc" className="mt-1 text-sm leading-relaxed text-slate-600">
        ...
        <div className="space-y-2 border-b border-slate-100 bg-slate-50/30 px-4 py-3 sm:px-5" dir="rtl">
          ...
          <dl className="grid gap-2 text-sm">
            <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1">
              <dt className="text-slate-500">النشاط</dt>
              <dd className="min-w-0 break-words font-medium text-slate-900">
                {row.program_name ?? `#${row.id}`}
              </dd>
            </div>
          </dl>
        </div>
