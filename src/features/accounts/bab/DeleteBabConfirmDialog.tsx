...
type Props = {
  open: boolean;
  row: BabWithRelations | null;
  isSubmitting: boolean;
  onClose: () => void;
  onConfirm: (id: number) => void;
};

export function DeleteBabConfirmDialog({ open, row, isSubmitting, onClose, onConfirm }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border bg-white p-4 shadow-xl" dir="rtl">
        <h3 className="font-semibold">تأكيد حذف الباب</h3>
        <p className="mt-2 text-sm">هل تريد حذف: <span className="font-semibold">{row.bab_name ?? `#${row.id}`}</span>؟</p>
